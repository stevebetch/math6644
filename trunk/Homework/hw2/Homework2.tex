...
$$
where $x^*$ is the solution, k is a constant, and $x_n,\ x_{n+1}$ are the values of $x$ at steps $n$ and $n+1$. Since $x^*$ is the solution, $x_n-x^*$ can be written as:
$$||e_{n+1}||\le k||e_{n}||^2\le(k||e_{n-1}||^2)^2\le\dots
$$
We know from these equations and the problem statement that the Newton iteration converges quadratically and that the norm of the error is dependent upon the previous step's error. Thus, to get to $x_n$, the error has converged by $
(k||x_0-x^*||^{2})^n=||e_n||$. The number of iterations necessary to get to the $\epsilon$ tolerance is:
$$\frac{e_n}{e_0}=(k||x_0-x^*||^2)^{n-1}=\epsilon
$$
$$n=\frac{log(\epsilon)}{log(k||e_0||^2)}+1
$$
...
$$\frac{e_n}{e_0}=(k||x_0-x^*||)^{n-1}=\epsilon
$$
$$n=\frac{log(\epsilon)}{log(k||e_0||)}+1
$$
